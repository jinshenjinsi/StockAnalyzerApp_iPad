...
app = Flask(__name__)
app.secret_key = os.environ.get('FLASK_SECRET_KEY', 'stock_analyzer_secret_key_2026')
...
def analyze_stock_enhanced(symbol):
    ...
    try:
        ...
        # 计算技术指标（使用新模块）
        technical_analysis = technical_indicators.analyze_all_indicators(df)
        ...
        return {
            ...
            "technical_indicators": technical_analysis  # 添加详细技术指标数据
        }
...
def api_technical_indicators(symbol):
    ...
    try:
        ...
        indicators = technical_indicators.analyze_all_indicators(df)
...
def api_alerts():
    ...
    if request.method == "POST":
        ...
        # 获取用户的所有预警
        alerts = alert_system.get_user_alerts(session.get("user_id", "default"))
        return jsonify({
            "success": True,
            "alerts": alerts
        })
...
def api_favorites():
    ...
    if request.method == "POST":
        try:
            ...
            favorites_manager.add_favorite(user_id, symbol, group)
        ...
        try:
            ...
            favorites_manager.remove_favorite(user_id, symbol, group)
            return jsonify({"success": True})
        except Exception as e:
            return jsonify({"success": False, "error": str(e)})
        ...
        # GET - 获取收藏夹
        groups = favorites_manager.get_user_favorites(user_id)
        return jsonify({
            "success": True,
            "groups": groups
        })

@app.route("/api/favorites/groups", methods=["POST", "DELETE"])
def api_favorite_groups():
    ...
    if request.method == "POST":
        try:
            ...
            favorites_manager.create_group(user_id, group_name)
            return jsonify({"success": True})
        except Exception as e:
            return jsonify({"success": False, "error": str(e)})
        ...
        try:
            data = request.get_json()
            group_name = data.get("group_name")
            favorites_manager.delete_group(user_id, group_name)
            return jsonify({"success": True})
        except Exception as e:
            return jsonify({"success": False, "error": str(e)})
